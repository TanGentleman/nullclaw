...
- `--llm-triage external` — 通过 `providers.completeWithSystem` 向已配置 LLM 提交 envelope。默认使用 `cfg.default_provider` 与 `cfg.default_model`，可用 `--llm-provider` / `--llm-model` 覆盖。若 provider 是本地的（如 Ollama），envelope 也不会离开本机
...
- 对隐私敏感场景，启用 `--llm-triage external` 前将 `default_provider` 固定为 `ollama` 等本地 provider
